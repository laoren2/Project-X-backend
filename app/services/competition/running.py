...
from app.schemas.competition.common import SportType, TeamRelationship, TeamStatus, RecordStatus
...
from app.db.models.competition import RunningEvent, RunningTrack, RunningSeason, RunningRaceRecord, RunningTeam, RunningTeamMember, RunningTeamAppliedMember
...
async def create_track_service(db: AsyncSession, track_form: RunningTrackCreateForm, image_url: str) -> RunningTrackBaseInfoInternal:
    ...
    if track is not None:
        raise BizException(code=ErrorCode.TRACK_ALREADY_EXIST, message="Running赛道已存在,不可重建创建")
...
async def query_leaderboard_in_page(
    ...
    if timestamp:
        key = f"leaderboard:running:{track_id}:{gender.value}:snapshot:{timestamp}"
    else:
...
# 生成排行榜快照
async def generate_leaderboard_snapshot(track_id: str, gender: Gender) -> str:
