...
# 比赛奖励结算
async def settle_match_result(db: AsyncSession, record: BikeRaceRecord, user: User) -> MatchFinishInfo | None:
    ...
    if record.status == RecordStatus.completed or record.status == RecordStatus.toBeVerified:
        ...
        total_rewards = []
        coin_balance = await reward_ccasset(db, CCAssetType.COIN, base_reward_coin + improvement_bonus_coin, user.id, "单次bike比赛结算", AssetOperation.REWARD)
        ...
        if reward_stone > 0:
            stone_balance = await reward_ccasset(db, reward_stone_type, reward_stone, user.id, "单次bike比赛结算", AssetOperation.REWARD)
