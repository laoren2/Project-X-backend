...
from app.schemas.competition import (
    BikeEventCreateForm,
    BikeEventBaseInfo, BikeEventUpdateForm, BikeEventBaseInfoInternal,
    BikeTrackBaseInfo, BikeTrackCreateForm,
    BikeTrackUpdateForm, BikeTrackBaseInfoInternal
)
from app.db.models import BikeEvent, BikeTrack
...
async def create_event_service(db: AsyncSession, event_form: BikeEventCreateForm, image_url: str) -> BikeEventBaseInfoInternal:
    ...
    season = await get_season_by_name_and_sport_type(db, event_form.season_name, "bike")
    if season is None:
...
async def query_events_by_sport_and_region(db: AsyncSession, region_name: str) -> List[BikeEventBaseInfo]:
    seasons = await get_season_now_by_sport(db, sport_type="bike")
    if not seasons:
...
async def create_track_service(db: AsyncSession, track_form: BikeTrackCreateForm, image_url: str) -> BikeTrackBaseInfoInternal:
    ...
    season = await get_season_by_name_and_sport_type(db, track_form.season_name, "bike")
    if season is None:
        raise BizException(code=ErrorCode.SEASON_NOT_FOUND, message="赛季不存在")
