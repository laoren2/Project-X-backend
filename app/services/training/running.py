...
from app.schemas.training.common import (
    ...
    RouteApplyStatus, GridOccupancyResponse, RegionGridOccupancyRankInfo, RegionGridOccupancyRankListResponse,
...
from app.crud.training.running import (
    ...
    ensure_running_effect_grids_generated, get_nearby_effect_grids, get_effect_grids_within_distance
)
...
import math, uuid, logging, random, json
...
async def finish_free_training_service(db: AsyncSession, info: FreeTrainingFinishInfo, user_id: str) -> FreeTrainingFinishResponse:
    async with db.begin():
        ...
        if info.client_upload_id:
            existing = await get_running_free_training_record_by_upload_id(db, user.id, info.client_upload_id)
...
async def apply_buff_grids(
    ...
    if passed_grids:
        ...
        effect_stmt = (
            ...
            .where(
                RunningEffectGrid.active_date == local_date,
                tuple_(RunningEffectGrid.grid_x, RunningEffectGrid.grid_y).in_(unique_grids),
...
async def apply_training_rewards(
    ...
    for point in path:
        ...
        if point.power:
            has_power = True
        if point.step_cadence:
            has_step_cadence = True
    ...
    if has_bpm or has_power or has_step_cadence:
...
# 根据训练路线，计算覆盖的网格，更新 user_grid_familiarity_running 表
async def update_user_familiarity(
    ...
    return await update_user_familiarity_by_grids(db, season_id, user_id, grid_region_pairs)
    ...
    if record.path and record.path.path:
        try:
            for point_data in record.path.path:
                # 注意兼容新旧数据格式
                path_points.append(RunningFreeTrainingPathPoint.model_validate(point_data))
        except Exception:
            logger.exception("Handle path data failed in querying running free training record detail info")
...
async def query_grid_info_service(
    ...
    stmt = (
        ...
        .where(
            RunningEffectGrid.active_date == local_date,
            RunningEffectGrid.grid_x >= min_x,
            RunningEffectGrid.grid_x <= max_x,
            RunningEffectGrid.grid_y >= min_y,
            RunningEffectGrid.grid_y <= max_y,
            RunningEffectGridHistory.id.is_(None)
        )
        .order_by(
            RunningEffectGrid.grid_x.asc(),
            RunningEffectGrid.grid_y.asc()
        )
    )
...
async def query_occupied_grids_count(
    ...
    if region_id is not None:
        ...
        result = await db.execute(text("""
            ...
            FROM running_region_grid_occupancies me
            LEFT JOIN running_region_grid_occupancies a
    ...
    sql = text(
        ...
        FROM user_grid_familiarity_running me
        WHERE me.season_id = :season_id
          AND me.user_id = :user_id
          AND NOT EXISTS (
            SELECT 1
            FROM user_grid_familiarity_running o
...
async def query_region_occupied_grids_ranklist(
    ...
    result = await db.execute(text(f"""
        ...
        FROM running_region_grid_occupancies r
...
async def query_me_familiarity_by_grid(
    ...
    sql_count = text(
        ...
        FROM user_grid_familiarity_running_agg
...
async def query_familiarity_ranking_by_grid(
    ...
    sql = text(
        ...
        FROM user_grid_familiarity_running_agg f
...
async def create_training_route_service(db: AsyncSession, user_id: str, data: CreateRouteRequest) -> CPAssetResponse:
    ...
    new_balance = await consume_cpasset(db, user.id, route_card_def.id, 1, "跑步路线创建")

    route = RunningTrainingRoute(
...
async def query_routes_service(
    ...
    next_cursor = None
    for row in rows:
        route = row["RunningTrainingRoute"]
        ...
        routes.append(RunningRouteInfo(
    ...
    return RunningRouteInfoResponse(
        routes=routes,
...
async def query_my_routes_service(
    ...
    page: int,
    size: int
) -> RunningRouteManageInfoResponse:
    user = await get_user_by_id(db, user_id)
    if user is None:
        raise BizException(code=ErrorCode.USER_NOT_FOUND, message="user.not_found")
...
async def finish_route_training_service(db: AsyncSession, finish_info: RouteTrainingFinishInfo, user_id: str) -> RouteTrainingFinishResponse:
    async with db.begin():
        ...
        if finish_info.client_upload_id:
            existing = await get_running_route_training_record_by_upload_id(db, user.id, finish_info.client_upload_id)
            if existing is not None:
        ...
        free_training_path = [RunningFreeTrainingPathPoint(
            base=p.base,
            power=p.power,
            step_cadence=p.step_cadence,
            vertical_amplitude=p.vertical_amplitude,
            touchdown_time=p.touchdown_time,
        ...
        for item in finish_info.bonus_in_cards:
            ...
            if card is not None:
                db.add(CardBonusInRunningRouteTrainingRecord(
                    record_id=record.id,
                    card_id=card.id,
                    bonus_time=item.bonus_time
                ))
        ...
        if should_update_rank:
            ...
            split_profile = build_split_profile(
                [p.base for p in finish_info.path], route.route_data, final_score, "running",
                checkpoint_events=checkpoint_events,
                card_bonus_samples=[
            ...
            if not route.is_premium:
                ...
                if overflow_ids:
                    await db.execute(
                        text(
                            """
                            DELETE FROM running_route_ranklists
                            WHERE id = ANY(:ids)
                            """
                        ),
                        {
...
async def query_route_training_record_detail_service(db: AsyncSession, lang: Language, record_id: str, viewer_id: str | None) -> RouteTrainingRecordDetailResponse:
    ...
    if record.path and record.path.path:
        try:
            for point_data in record.path.path:
                # 注意兼容新旧数据格式
                path_points.append(RunningRouteTrainingPathPoint.model_validate(point_data))
        except Exception:
            logger.exception("Handle path data failed in querying running route training record detail info")
...
async def query_route_ranklist_service(
    ...
    sql = text(
        ...
        WITH ranked AS (
            ...
            FROM running_route_ranklists r
            JOIN users u ON u.id = r.user_id
    ...
    for _, row in enumerate(rows):
        rank_infos.append(RunningRouteRankInfo(
            rank=row.rank,
    ...
    return RunningRouteRanklistResponse(
        ranklist=rank_infos,
        next_cursor=next_cursor
    )

async def query_route_ranklist_me_service(db: AsyncSession, user_id: str, route_id: str) -> RunningRouteRankInfo | None:
    user = await get_user_by_id(db, user_id)
    if user is None:
        raise BizException(code=ErrorCode.USER_NOT_FOUND, message="user.not_found")
    route = await get_route_by_route_id(db, route_id)
