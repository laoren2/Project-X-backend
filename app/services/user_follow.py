...
from app.schemas.user import UserRelationInfo, UserStatus
...
async def follow_user(db: AsyncSession, follower_id, followed_id):
    if follower_id == followed_id:
        raise BizException(code=ErrorCode.USER_FOLLOW_SELF, message="user.follow_self")
    user_follower = await get_user_by_id(db, follower_id)
    user_followed = await get_user_by_id(db, followed_id)
    if user_followed is None or user_followed.status == UserStatus.deleted:
        raise BizException(code=ErrorCode.USER_NOT_FOUND, message="user.not_found")
    already_following = await user_follow.is_following(db, user_follower.id, user_followed.id)
    if already_following:
        raise BizException(code=ErrorCode.USER_FOLLOW_REPEAT, message="user.repeat_follow")
...
async def cancel_follow_user(db: AsyncSession, follower_id, followed_id):
    if follower_id == followed_id:
        raise BizException(code=ErrorCode.USER_FOLLOW_SELF, message="user.cancel_follow_self")
    user_follower = await get_user_by_id(db, follower_id)
    user_followed = await get_user_by_id(db, followed_id)
    if user_followed is None:
        raise BizException(code=ErrorCode.USER_NOT_FOUND, message="user.not_found")
