...
async def _lock_bike_occupancy_grids(
    ...
    await db.execute(text("""
        SELECT pg_advisory_xact_lock(hashtext(
            'bike-grid-occupancy:' || :season_id || ':' || g.grid_x || ':' || g.grid_y
        ))
        FROM (
...
async def _refresh_bike_occupancy_projection(
    db: AsyncSession, season_id: uuid.UUID, grid_region_pairs: list[tuple[int, int, uuid.UUID]]
) -> None:
    ...
    await db.execute(text("""
        WITH input_grids AS (
            ...
            FROM bike_grid_occupancy_owners o JOIN input_grids i
            ...
            FROM user_grid_familiarity_bike f JOIN input_grids i
            ...
            INSERT INTO bike_grid_occupancy_owners
            ...
            INSERT INTO bike_region_grid_occupancies
            ...
            SET occupied_count = bike_region_grid_occupancies.occupied_count + EXCLUDED.occupied_count,
                updated_at = NOW()
        )
        DELETE FROM bike_region_grid_occupancies
...
async def update_user_familiarity_by_grids(
    ...
    # These locks are intentionally acquired before changing familiarity: concurrent uploads
    # otherwise could both select a winner from an outdated transaction snapshot.
    await _lock_bike_occupancy_grids(db, season_id, grid_region_pairs)
    sql = text(
        """
        WITH input_grids AS (
            SELECT
    ...
    row = result.first()
    await _refresh_bike_occupancy_projection(db, season_id, grid_region_pairs)
