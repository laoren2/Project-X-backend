...
async def get_bonus_record_with_team_magic_card_by_team_id(db: AsyncSession, team_id: uuid.UUID) -> List[RunningBonusByTeamMember]:
    result = await db.execute(
...
async def get_bonus_record_with_team_magic_card_by_team_user(db: AsyncSession, team_id: uuid.UUID, user_id: uuid.UUID) -> RunningBonusByTeamMember | None:
    result = await db.execute(
        select(RunningBonusByTeamMember)
        .where(
            RunningBonusByTeamMember.team_id == team_id,
            RunningBonusByTeamMember.user_id == user_id
        )
    )
    return result.scalar_one_or_none()
...
async def get_user_best_race_profile(db: AsyncSession, track_internal_id: uuid.UUID, user_id: uuid.UUID) -> dict | None:
    ...
    result = await db.execute(
        select(RunningRaceRecord.split_profile)
        .where(
            RunningRaceRecord.track_id == track_internal_id,
            RunningRaceRecord.user_id == user_id,
            RunningRaceRecord.status == RecordStatus.completed,
            RunningRaceRecord.split_profile.isnot(None)
        )
        .order_by(RunningRaceRecord.duration_seconds.asc())
        .limit(1)
    )
    return result.scalar_one_or_none()
