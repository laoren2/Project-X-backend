...
from app.schemas.training.bike import (
    ...
    BikeGridTileResponse, BikeGridInfoResponse, RouteTrackApplyRequest, BikeNearbyGridsResponse
)
from app.schemas.training.common import (
    RegionExploreResponse, GridTileRequest, GridFamiliarityMeResponse,
    GridFamiliarityRankListResponse, RouteSortType, GridOccupancyResponse, RegionGridOccupancyRankListResponse,
    WeeklyTrainingSummaryResponse
)
...
# 查询附近的奖励网格（运动中雷达指引）
@router.get("/query_nearby_grids",response_model=BaseResponse[BikeNearbyGridsResponse],summary="查询附近的奖励网格")
...
# 查询我创建的路线
@router.get("/routes/my",response_model=BaseResponse[BikeRouteManageInfoResponse],summary="查询我创建的路线")
async def query_routes(
    page: int = Query(...),
    size: int = Query(...),
    auth: AuthContext = Depends(get_current_user),
