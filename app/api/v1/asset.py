...
from app.services.asset_manage import (
    ...
    buy_ccasset_use_ccasset
)
from app.schemas.asset import (
    CCAssetsResponse, CPAssetsResponse, CPAssetBuyRequest, CC_CC_PurchaseResultResponse,
...
async def query_user_cpasset(
    ...
    asset = await get_user_cpasset(db, auth.payload["user_id"], asset_id, lang)
    return BaseResponse.success(token=auth.new_token, data=asset)

# 购买ccasset
@router.post("/buy_ccasset",response_model=BaseResponse[CC_CC_PurchaseResultResponse],summary="购买通用货币资产")
async def buy_ccasset(
    request: CC_CC_BuyRequest,
    auth: AuthContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    result = await buy_ccasset_use_ccasset(db, auth.payload["user_id"], request.buy, request.amount, request.use)
...
async def buy_cpasset(
    ...
    result = await buy_cpassets_use_ccasset(db, auth.payload["user_id"], request.cpasset_id, request.cpamount)
    return BaseResponse.success(token=auth.new_token, data=result, message="购买成功")
...
async def query_equip_card_shop_detail(
    ...
    card = await get_equip_card_shop_detail_service(db, lang, def_id)
...
async def query_user_equip_card_detail(
    ...
    card = await get_user_equip_card_detail_service(db, lang, card_id)
    return BaseResponse.success(token=auth.new_token, data=card)
...
async def buy_equip_card(
    ...
    result = await buy_equip_card_use_ccasset(db, lang, auth.payload["user_id"], card_def_id)
    return BaseResponse.success(token=auth.new_token, data=result, message="购买成功")
...
async def upgrade_equip_card_mat(
    ...
    result = await upgrade_equip_card_mat_service(db, lang, auth.payload["user_id"], card_id)
...
async def upgrade_equip_card_fusion(
    ...
    result = await upgrade_equip_card_fusion_service(db, lang, card_id, fusion_card_id)
    return BaseResponse.success(token=auth.new_token, data=result, message="升级成功")
...
async def upgrade_equip_card_skill1(
    ...
    result = await upgrade_equip_card_skill1_service(db, lang, auth.payload["user_id"], card_id)
    return BaseResponse.success(token=auth.new_token, data=result, message="升级成功")
...
async def upgrade_equip_card_skill2(
    ...
    result = await upgrade_equip_card_skill2_service(db, lang, auth.payload["user_id"], card_id)
    return BaseResponse.success(token=auth.new_token, data=result, message="升级成功")
...
async def upgrade_equip_card_skill3(
    ...
    result = await upgrade_equip_card_skill3_service(db, lang, auth.payload["user_id"], card_id)
    return BaseResponse.success(token=auth.new_token, data=result, message="升级成功")
