...
async def login_sms(data: schemas_user.SMSCodeVerify, db: AsyncSession = Depends(get_db)):
    ...
    token, user, isRegister, role = await login_or_register(db, data.phone_number, data.timezone)
...
async def login_email(data: schemas_user.EmailCodeVerify, db: AsyncSession = Depends(get_db)):
    ...
    token, user, isRegister, role = await login_or_register_email(db, data.email_address, data.timezone)
    relation = await get_relation_count(db, user.user_id)
    return BaseResponse.success(token=token, message="登录成功", data=schemas_user.LoginResponse(user=user, relation=relation, role=role, isRegister=isRegister))
...
@router.post("/update_record_visibility", response_model=BaseResponse[schemas_user.RecordVisibility], summary="更新比赛结果可见范围")
async def update_record_visibility(
    visibility: schemas_user.RecordVisibility = Query(...),
    auth: schemas_user.AuthContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    result = await update_record_visibility_service(visibility, auth.payload["user_id"], db)
    return BaseResponse.success(token=auth.new_token, message="更新成功", data=result)
...
@router.post("/realname", response_model=BaseResponse[None], summary="实名认证")
async def realname(
    form: schemas_user.RealNameForm = Depends(schemas_user.RealNameForm.as_form),
    front_image: UploadFile = File(...),
    auth: schemas_user.AuthContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    front_bytes = await front_image.read()
    await realname_service(db, auth.payload["user_id"], front_bytes, form.country_code, form.method)
...
async def login_with_apple(
    ...
    if not apple_sub:
        raise BizException(code=ErrorCode.APPLE_ID_ERROR, message="identity.verify_failed.apple")
    new_token, user, is_register, role = await login_or_register_apple(db, apple_sub, email, jws.timezone)
    relation = await get_relation_count(db, user.user_id)
