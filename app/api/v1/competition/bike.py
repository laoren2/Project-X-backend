...
from app.schemas.competition.bike import (
    ...
    BikeTracksUserInfoRequest, BikeTracksUserInfoResponse
)
...
from app.services.competition.bike import (
    ...
    get_track_pace_baseline_service, get_record_pace_snapshot_service
)
...
@router.post("/query_tracks_user_info", response_model=BaseResponse[BikeTracksUserInfoResponse], summary="批量查询赛道用户态信息")
async def query_tracks_user_info(
    request: BikeTracksUserInfoRequest,
    auth: AuthContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    infos = await query_tracks_user_info_service(db, auth.payload["user_id"], request.track_ids)
    return BaseResponse.success(token=auth.new_token, data=BikeTracksUserInfoResponse(infos=infos))
